---
title: "NIfTI Basics using the Eve atlas"
author: "John Muschelli"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{NIfTI Basics using the Eve atlas}
...
Here we will use imaging data from the `oro.nifti` package:
...
We can view the flipped T1 alongside the original image:
...
In some instances, there are extraneous slices to an image.  For example, in the image we read in, there are extraneous background areas.  For plotting or further analyses, we can drop these empty dimensions using the `neurobase::dropEmptyImageDimensions` function or `drop_empty_dim` shorthand function.  
